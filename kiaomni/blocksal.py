...
BlockSal groups evictable prompt positions into fixed-size blocks, scores each
block by mean token saliency, and keeps the highest-scoring blocks while always
protecting attention sinks and the recency tail.

The historical implementation could cross the requested budget by up to one
block. This reusable selector keeps the same block-ranking rule but uses a
single partial boundary block when necessary so benchmark budgets are exact.
"""
...
def select_blocksal_keep(
    ...
    *,
    ...
    """Select exactly budget prompt positions using BlockSal.

    Blocks are defined in absolute prompt coordinates and scored by the mean
    saliency of their currently evictable members. Full blocks are admitted in
    descending score order. If the last admitted block does not fit the exact
    remaining budget, the highest-saliency tokens inside that block fill the
    residual slots. This preserves the historical block-ranking semantics
    while making B=98 directly comparable with exact-budget methods.
    """
    if saliency.ndim != 1:
        raise ValueError(f"saliency must be 1-D, got shape={saliency.shape}")
    ...
    if remaining <= 0 or free.size == 0:
        return BlockSalSelection(
            keep_indices=np.asarray(sorted(protected), dtype=np.int64),
            block_size=block_size,
            partial_boundary_block=False,
            protected_tokens=len(protected),
        )
