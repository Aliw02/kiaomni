"""
037_falcon3_benchmark.py — Full Policy Benchmark on Falcon3-7B-Instruct
=========================================================================
Platform : L4 (24 GB VRAM)
Model    : tiiuae/Falcon3-7B-Instruct (NF4 4-bit, bfloat16 compute)
...
Run:
    python experiments/037_falcon3_benchmark.py

Outputs:
    experiments/results/037_falcon3_results/results.json
    experiments/results/037_falcon3_results/predictions.csv
    experiments/results/037_falcon3_results/speed_vram.csv
    experiments/results/037_falcon3_results/eviction_coherence_loss.csv
    experiments/results/037_falcon3_results/checkpoints/
"""
...
OUT_DIR = Path("results/037_falcon3_results")
...
def get_ratio_adaptive_sigma(sal: np.ndarray, budget: int, seq_len: int) -> int:
    ...
    return int(max(1, round(compression_ratio * peakiness)))
...
def kiaomni_ratio_adaptive_keep(sal: np.ndarray, budget: int, seq_len: int) -> set:
    ...
    print(f"  RatioAdaptive sigma={sigma}", flush=True)
    return kiaomni_fixed_keep(sal, budget, seq_len, sigma)
        ...
        for ctx_len in CTX_LENS:
            ...
            for trial in range(N_TRIALS):
                ...
                if done % 5 == 0:
                    ...
                    ki = np.mean(results["ruler"][task_name][ctx_len]["KiaOmni_s8"][256]["f1"] or [0])
