...
4 conditions:
  KiaOmni_σ8   + sal_mean     (natural — original condition)
  KiaOmni_σ8   + sal_snapkv   (swapped — KiaOmni under SnapKV signal)
  RealSnapKV   + sal_snapkv   (natural — original condition)
  RealSnapKV   + sal_mean     (swapped — SnapKV under KiaOmni signal)

Scope:
  Model   : Qwen/Qwen2.5-7B-Instruct  (4-bit NF4)
  Context : 4096
  Budgets : [98, 128]  (2.4% and 3.1% compression ratio)
  Tasks   : niah_single, niah_multikey, vt  (RULER only)
  Trials  : 15 per cell
  Total   : 3 tasks × 15 trials × 4 conditions × 2 budgets = 360 runs

Interpretation guide (see bottom of file):
  If KiaOmni wins both natural AND swapped  → selector is stronger
  If KiaOmni only wins on natural           → signal-dependent
  If SnapKV improves on KiaOmni signal      → pipeline bias exists

Platform : Kaggle T4×1 free (~2-3 hours)
Run      : !python 039_swap_experiment.py

Outputs  : 039_swap_results/results.json
           039_swap_results/predictions.csv
           039_swap_results/checkpoints/
"""
...
CTX_LEN     = 4096
BUDGETS     = [98, 128]   # two budgets: 2.4% and 3.1% compression of 4K
N_TRIALS    = 15
...
# condition name → (selector_name, signal_key)
CONDITIONS = {
...
# ═══════════════════════════════════════════════════════════════════════════════
# SALIENCY — identical to 033, single forward pass
# ═══════════════════════════════════════════════════════════════════════════════
...
def select_kiaomni(sal: np.ndarray, budget: int, seq_len: int) -> set:
    """KiaOmni_σ8 selector: boxcar-smoothed log1p saliency, page-granular eviction."""
...
def select_snapkv(sal: np.ndarray, budget: int, seq_len: int) -> set:
    """RealSnapKV selector: per-head max-pool top-K then union, budget-enforced trim."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# RULER TASK BUILDERS  (identical to 033)
# ═══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    # results[task][budget][condition] = {metric: []}
    results: dict = {
        task: {
    ...
    for task in RULER_TASKS:
        for trial in range(N_TRIALS):
            ...
            # Single forward pass — both signals
            try:
            ...
            # Run all 4 conditions × 2 budgets
            for B in BUDGETS:
                trial_data[str(B)] = {}
