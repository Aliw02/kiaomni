"""
038_biomistral_comparison.py — KV-Cache Eviction on Biomedical LLM
===================================================================
Platform : Modal L4 (24 GB VRAM)
Model    : BioMistral/BioMistral-7B (NF4 4-bit, bfloat16 compute)
           Mistral-7B fine-tuned on PubMed Central — generative decoder,
           fully open, no authentication required, citable (arXiv:2402.10373)

Policies : SnapKV_Modified, RealSnapKV, Ada-SnapKV, H2O,
           KiaOmni_σ8, KiaOmni_Adaptive, KiaOmni_RatioAdaptive, KiaOmni_Quest,
           KiaOmni_Gaussian, KiaOmni_AnchorExp, KiaOmni_Scissorhands

Bio-RULER Tasks (synthetic, real clinical identifiers):
...
Bio-LB Tasks (real open datasets via HuggingFace):
  pubmedqa         — PubMedQA pqa_labeled (yes/no/maybe, long biomedical context)
  pubmedqa_long    — PubMedQA pqa_unlabeled (longer contexts, descriptive answer)
  medmcqa          — MedMCQA medical MCQ (Indian medical exams, 4 choices)
  medalpaca_medqa  — Medical Meadow MedQA (USMLE-style questions)
  medalpaca_wiki   — Medical Meadow WikiDoc medical QA
  clinical_niah    — Synthetic clinical report NIAH (ICD code as needle)
...
Paper Claim Tested:
  KiaOmni σ=8 preserves clinically critical multi-subword identifiers
  (rsXXXXXXXX, GENE c.XXXXX, ICD codes) better than pointwise methods
  because the boxcar kernel fills intra-identifier subword gaps.
  Tested on a biomedical-domain fine-tuned model with real clinical benchmarks.
"""
...
CHAIN_LEN   = 3   # drug interaction chain length

IN_MODAL = os.environ.get("MODAL_IMAGE_ID") is not None
if IN_MODAL:
    OUT_DIR = Path("/vol/kv_results/038_biomistral_results")
else:
    OUT_DIR = Path("038_biomistral_results")
...
# ═══════════════════════════════════════════════════════════════════════════════
# BIOMEDICAL NEEDLE POOLS (real clinical identifiers — publicly documented)
# ═══════════════════════════════════════════════════════════════════════════════

# Real dbSNP rsIDs with clinical significance (from ClinVar/PharmGKB)
SNP_POOL = [
    ("rs7412",    "APOE",   "Alzheimer's risk / lipid metabolism"),
    ("rs429358",  "APOE",   "Alzheimer's risk (ε4 allele)"),
...
# Real HGVS gene mutations (from ClinVar, well-documented pathogenic variants)
GENE_MUTATION_POOL = [
...
# Real drug pairs with documented interactions (from DrugBank / FDA labels)
...
# ICD-10 codes for clinical NIAH
ICD_POOL = [
...
def load_model():
    ...
    # Do NOT pass device_map or torch_dtype — bitsandbytes places model on CUDA automatically.
...
# ═══════════════════════════════════════════════════════════════════════════════
# SALIENCY — standard separate q_proj / k_proj (Mistral architecture)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# EVICTION POLICIES (identical to 033/037)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# BIO-RULER TASK BUILDERS
# ═══════════════════════════════════════════════════════════════════════════════

def build_bio_niah_single(rng: random.Random, tok, target_toks: int, depth: float):
    """Needle: real dbSNP rsID buried in genomic literature haystack."""
...
def build_bio_niah_gene(rng: random.Random, tok, target_toks: int, depth: float):
    """Needle: real HGVS gene mutation (GENE + cDNA notation) in clinical text."""
...
def build_bio_vt(rng: random.Random, tok, target_toks: int):
    """Drug interaction chain: Drug_A inhibits Drug_B which affects Drug_C outcome."""
    ...
    final_drug_b = pairs[0][1]
    answer = pairs[0][1]  # the affected drug
...
def build_clinical_niah(rng: random.Random, tok, target_toks: int, depth: float):
    """Needle: ICD-10 code buried in clinical documentation filler."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# BIO-LB DATASET LOADERS (HuggingFace datasets — all open/no-auth)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if task == "pubmedqa":
        ...
        for s in raw:
            ...
            out.append({
                "context":  context,
                "question": s.get("question", ""),
                "answer":   long_ans[:200],  # first 200 chars as reference
            })
            if len(out) >= n:
                break
        return out
        ...
        # Synthetic clinical NIAH — use ICD pool, no external dataset needed
...
def build_bio_lb_ids(sample: dict, task: str, tok, max_ctx: int) -> torch.Tensor:
    ...
    # For clinical_niah, embed the ICD needle into the context
    ...
    # Truncate context to fit max_ctx
        ...
        for ctx_len in CTX_LENS:
            ...
            for trial in range(N_TRIALS):
                ...
                if done % 5 == 0:
                    ...
                    ki = np.mean(results["bio_ruler"][task_name][ctx_len]["KiaOmni_σ8"][256]["contains"] or [0])
