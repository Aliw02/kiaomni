...
Method   : Sliding window evaluation — each chunk is a fixed context window.
           Saliency is computed once per chunk; eviction applied before
           the forward pass used for loss computation.
Budgets  : {98, 128, 256, 512, FullContext}
Chunks   : 50 non-overlapping chunks of CTX_LEN tokens

Install:
    !pip install -U -q transformers bitsandbytes accelerate datasets

Run:
    !python 035_ppl_wikitext2.py

Outputs:
    035_ppl_results/ppl_table.json    — policy × budget → PPL
    035_ppl_results/ppl_table.csv
"""
...
CTX_LEN    = 4096          # tokens per chunk
N_CHUNKS   = 50            # number of WikiText-2 chunks to evaluate
BUDGETS    = [98, 128, 256, 512]
STRIDE     = CTX_LEN      # non-overlapping
...
def load_wikitext2_tokens(tok, n_chunks: int, ctx_len: int) -> list[torch.Tensor]:
    """Download WikiText-2 test split and tokenise into fixed-length chunks."""
    try:
        from datasets import load_dataset
        ds = load_dataset("wikitext", "wikitext-2-raw-v1", split="test")
        text = "\n\n".join(ds["text"])
    except Exception:
        # Fallback: download raw file directly
...
# ═══════════════════════════════════════════════════════════════════════════════
# SALIENCY  (last-layer hook, same as 033/035_passkey)
# ═══════════════════════════════════════════════════════════════════════════════
...
def compute_ppl(model, ids: torch.Tensor, keep: set | None = None) -> float:
    """Cross-entropy loss → perplexity on the token sequence after eviction."""
    device = next(model.parameters()).device
    ids    = ids.to(device)
    if keep is not None:
...
def main() -> None:
    ...
    # ppl_scores[policy][budget] = [ppl per chunk]
