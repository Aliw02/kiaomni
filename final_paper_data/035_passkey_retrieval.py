...
=======================================================
Platform : Kaggle T4×2 / Modal A100
Model    : configurable via MODEL_NAME (default: Phi-3.5-mini)
...
Depths   : {0.1, 0.25, 0.5, 0.75, 0.9}  (position in context)
...
Run:
    !python 035_passkey_retrieval.py

Outputs:
    035_passkey_results/results.json
    035_passkey_results/accuracy_table.csv   — policy × budget × depth
    035_passkey_results/checkpoints/
"""
...
MODEL_NAME  = "Qwen/Qwen2.5-7B-Instruct"   # change to Phi/Mistral/Llama as needed
...
OUT_DIR  = Path("035_passkey_results")
...
# ═══════════════════════════════════════════════════════════════════════════════
# SALIENCY  (mirrors 033 hook-based extraction, last-layer only for speed)
# ═══════════════════════════════════════════════════════════════════════════════
...
def extract_saliency(ids: torch.Tensor, model) -> dict:
    """Returns sal_snapkv (nh,L), sal_mean (L,), and sal_scissor (L,) from last transformer layer.
    sal_scissor is the mean of saliency from layers L/4, L/2, L-1 for Scissorhands."""
    c    = model.config
    ...
    # sal_scissor: mean of saliency at layers L/4, L/2, L-1
...
def kiaomni_gaussian_keep(sals: dict, budget: int, seq_len: int) -> set:
    """Gaussian smoothing + log1p for more robust selection."""
...
def kiaomni_scissorhands_keep(sals: dict, budget: int, seq_len: int) -> set:
    """Multi-layer saliency (L/4, L/2, L-1) - no smoothing, raw selection."""
...
def snapkv_modified_keep(sals: dict, budget: int, seq_len: int) -> set:
    """Block-level eviction with Mean/Max hybrid - modified SnapKV."""
    sal      = sals["sal_mean"]
...
def h2o_keep(sals: dict, budget: int, seq_len: int) -> set:
    """Raw top-k selection without smoothing - H2O baseline."""
    sal  = sals["sal_mean"]
    prot = _protected(seq_len)
    eff  = max(0, budget - len(prot))
    if budget >= seq_len:
...
POLICIES = {
    "FullContext":          None,
    "KiaOmni_σ8":           lambda s, B, L: kiaomni_keep(s, B, L, sigma=8),
...
def score(pred: str, key: str) -> bool:
    """Exact match on the digit string anywhere in the prediction."""
...
def main() -> None:
    ...
    # results[ctx][depth][policy][budget] = [bool, ...]
    results: dict = {
