...
Install:
    !pip install -U -q transformers bitsandbytes accelerate matplotlib

Run:
    !python 035_niah_heatmap.py

Outputs:
    035_heatmap_results/heatmap_{policy}_B{budget}.png
    035_heatmap_results/grid_data.json
    035_heatmap_results/grid_data.csv
    035_heatmap_results/checkpoints/
"""
...
OUT_DIR  = Path("035_heatmap_results")
...
# ═══════════════════════════════════════════════════════════════════════════════
# NEEDLE BUILDER  (same as 033 niah_single)
# ═══════════════════════════════════════════════════════════════════════════════
...
def extract_saliency(ids: torch.Tensor, model) -> dict:
    ...
    L   = ids.shape[1]
    buf: dict = {}
    layer_sals: dict = {}   # layer_idx -> sal_mean (L,)
    hooks = []
...
def main() -> None:
    ...
    # results[pol][budget][ctx][depth] = [bool, ...]
    results: dict = {
