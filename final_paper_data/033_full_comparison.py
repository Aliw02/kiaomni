"""
033_full_comparison.py — Full 6-Policy KV-Cache Benchmark
==========================================================
Platform : Kaggle T4×2 (32 GB VRAM total, device_map=auto)
Policies : SnapKV, H2O, KiaOmni_σ8, KiaOmni_Adaptive, PyramidKV, AdaKV
Tasks    : RULER (niah_single, niah_multikey, vt)
           + LongBench (qasper, hotpotqa, multifieldqa_en)
Contexts : {4096, 8192, 16384}  (16K needs Flash Attention 2)
Budgets  : {128, 256, 512}
N        : 15 RULER trials / 15 LongBench samples per cell
Metrics  : Token-F1, EM, ROUGE-L, Contains, PPL, tokens/sec,
           VRAM peak (saliency + generation), saliency latency
 
Install (Kaggle cell, run once then restart kernel):
    !pip install -U -q transformers bitsandbytes accelerate rouge-score
    !pip install flash-attn --no-build-isolation -q

Run:
    !python 033_full_comparison.py

Checkpointing: saves after every trial. Safe to interrupt and resume.

Outputs:
    033_full_comparison_results/results.json       — aggregated metrics
    033_full_comparison_results/predictions.csv    — raw predictions + judge columns
    033_full_comparison_results/speed_vram.csv     — per-trial timing + VRAM
    033_full_comparison_results/eviction_coherence_loss.csv — per-trial coherence loss
    033_full_comparison_results/checkpoints/       — per-trial recovery
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# CONFIG  (edit these to change scope of the run)
# ═══════════════════════════════════════════════════════════════════════════════

MODEL_NAME   = "Qwen/Qwen2.5-7B-Instruct"
CTX_LENS     = [4096, 8192, 16384]
BUDGETS      = [98, 128, 256, 512]
N_TRIALS     = 15          # RULER: trials per (task, ctx, budget) cell
LB_SAMPLES   = 15          # LongBench: samples per (task, ctx) pair
...
# KV-cache structural params
N_SINK      = 16
RECENCY     = 32
BLOCK_SIZE  = 16
SIGMA_FIXED = 8
SIGMA_MAX   = 64    # Adaptive KiaOmni σ_max — Qwen-calibrated from 015v3
SNAP_POOL_K = 5     # RealSnapKV 1D max-pooling kernel (arXiv:2404.14469 default)
SNAP_OBS_W  = 32    # RealSnapKV observation window size

# RULER task params
N_KEYS      = 4    # niah_multikey: number of needles
CHAIN_LEN   = 5    # vt: variable chain length

OUT_DIR  = Path("033_full_comparison_results")
...
COHERENCE_COLS = ["source", "task", "ctx", "trial_or_sample", "policy", "budget", "eviction_coherence_loss"]
...
# ═══════════════════════════════════════════════════════════════════════════════
# SALIENCY — single forward pass yields all signals
# ═══════════════════════════════════════════════════════════════════════════════

def extract_all_saliency(ids: torch.Tensor, model) -> dict:
    """
    One forward pass, three saliency signals:

    sal_mean      : np.ndarray[L]          last-layer, mean over heads
                                           → SnapKV / H2O / KiaOmni variants
    sal_per_head  : np.ndarray[H, L]      last-layer, per head
                                           → AdaKV per-head entropy budget
    sal_per_layer : np.ndarray[n_layers,L] mean-head saliency at every layer
                                           → real PyramidKV per-layer SnapKV
    """
    L_seq = ids.shape[1]
    c     = model.config
    ...
    # Accumulators
    last_k_buf: dict = {}
    # sal_per_layer[l] = mean-head saliency at layer l, shape (L,)
    sal_per_layer_list: list = [None] * n_layers

    hooks = []

    # ── All-layer hook: captures per-layer mean-head saliency ────────────────
    for l_idx, layer in enumerate(model.model.layers):
        ...
        def _make_k_hook(layer_idx, is_last, q_store):
            def _h(m, inp, out):
                ...
                # Last-query saliency (used by all non-SnapKV policies)
                sc2 = torch.matmul(q2[:, :, -1:, :], k2.transpose(-2, -1)) * (_hd2 ** -0.5)
                sal_heads = torch.softmax(sc2, dim=-1)[0, :, 0, :]  # (nh, L)
                sal_mean_l = sal_heads.mean(0).numpy()               # (L,)
                sal_per_layer_list[layer_idx] = sal_mean_l.astype(np.float32)
                if is_last:
                    last_k_buf["sal_heads"] = sal_heads.numpy()      # (nh, L)
                    # ── Real SnapKV voting (arXiv:2404.14469 §4) ─────────────
                    # Use last SNAP_OBS_W queries, sum attention over prefix only
                    obs_w = min(SNAP_OBS_W, _L)
                    q_obs = q2[:, :, -obs_w:, :]                     # (1, nh, obs_w, hd)
                    sc_obs = torch.matmul(q_obs, k2.transpose(-2, -1)) * (_hd2 ** -0.5)
                    # sc_obs: (1, nh, obs_w, L) — restrict voting to prefix tokens
                    prefix_len = max(1, _L - obs_w)
                    attn_prefix = torch.softmax(
                        sc_obs[..., :prefix_len], dim=-1)            # (1, nh, obs_w, prefix_len)
                    votes = attn_prefix.sum(dim=-2)                  # (1, nh, prefix_len)
                    # Pad observation window tokens with max vote (always keep)
                    max_v = votes.max(dim=-1, keepdim=True).values   # (1, nh, 1)
                    pad   = max_v.expand(1, _nh2, obs_w)             # (1, nh, obs_w)
                    sal_snapkv_h = torch.cat([votes, pad], dim=-1)[0]  # (nh, L)
                    last_k_buf["sal_snapkv"] = sal_snapkv_h.numpy()  # (nh, L)
    ...
    sal_heads_last  = last_k_buf.get("sal_heads")    # (nh, L) or None
    sal_snapkv_last = last_k_buf.get("sal_snapkv")   # (nh, L) or None

    if sal_heads_last is None:
        sal_mean = _last_layer_saliency(ids, model)
        sal_per_head = np.tile(sal_mean, (nh, 1))
    else:
        sal_per_head = sal_heads_last.astype(np.float32)   # (nh, L)
        sal_mean     = sal_per_head.mean(0)                # (L,)

    # Keep per-head shape (nh, L) — official SnapKV does per-head top-K, not averaged
    sal_snapkv = (
    ...
    # Build per-layer array [n_layers, L]; fall back to sal_mean if a hook missed
    sal_per_layer = np.stack(
        [(x if x is not None else sal_mean) for x in sal_per_layer_list], axis=0
    ).astype(np.float32)   # (n_layers, L)

    # 3-layer average for Scissorhands (no extra forward pass needed)
    ...
    return {
        "sal_mean":      sal_mean,
        "sal_snapkv":    sal_snapkv,     # obs-window voted scores (real SnapKV signal)
...
def _last_layer_saliency(ids: torch.Tensor, model) -> np.ndarray:
    """Fallback: last-layer mean-head saliency (O(L) memory)."""
    buf: dict = {}
...
def _boxcar(x: np.ndarray, sigma: int) -> np.ndarray:
    """O(N) prefix-sum boxcar average. sigma=0 → identity."""
    if sigma <= 0:
...
# ── 1b. RealSnapKV (arXiv:2404.14469 §4) ────────────────────────────────────
def snapkv_real_keep(sal: np.ndarray, budget: int, seq_len: int) -> set:
    """True SnapKV (arXiv:2404.14469): per-head top-K then union, matching official repo.

    sal shape: (n_heads, seq_len) — one saliency vector per attention head.
    Each head independently selects its top-K tokens; the union is the kept set.
    """
...
def unknow_kvCache(sal: np.ndarray, budget: int, seq_len: int) -> set:
    from scipy.ndimage import maximum_filter1d
    if budget >= seq_len:
        return set(range(seq_len))
    prot = _protected(seq_len)
...
def get_ratio_adaptive_sigma(sal: np.ndarray, budget: int, seq_len: int) -> int:
    ...
    # The pure relative compression distance instead of arbitrary max:
    compression_ratio = seq_len / max(1, budget)
    adaptive_sigma = compression_ratio * peakiness
    return int(max(1, round(adaptive_sigma)))

# ── Ada-SnapKV (adaptive budget based on observation window entropy) ──────────────────
...
# ── 1. Quest-Inspired Envelope (Max-Pooling) ──
def _quest_envelope(x: np.ndarray, sigma: int) -> np.ndarray:
    if sigma <= 0:
        return x.astype(np.float32)
    from scipy.ndimage import maximum_filter1d
    # نرفع التوكنات المحيطة لمستوى الإبرة بدلاً من تذويب الإبرة
...
# ── 2. Gaussian Smoothing (Signal Processing) ──
def _gaussian_smooth(x: np.ndarray, sigma: float) -> np.ndarray:
    if sigma <= 0:
        return x.astype(np.float32)
    from scipy.ndimage import gaussian_filter1d
    # تنعيم غاوسي يحافظ على القمة ويصنع تلاً بدلاً من هضبة
...
# ── 3. Anchor-Expand (Region of Interest / CV) ──
def kiaomni_anchor_expand_keep(sal: np.ndarray, budget: int, seq_len: int, radius: int = 5) -> set:
    ...
    cands = np.array([i for i in range(seq_len) if i not in prot])
    if len(cands) == 0: return prot
    
    # لا تنعيم: نأخذ القمم الصافية (المراسي) ونرتبها
    ...
    for anchor in sorted_idx:
        if len(keep_set) >= budget:
            break
        # بناء منطقة حماية ديناميكية حول المرساة
...
def qk_saliency_scissorhands(ids: torch.Tensor, model) -> np.ndarray:
    """يستخرج الانتباه من 3 طبقات (الربع، النصف، الأخيرة) لضمان استمرارية الأهمية دون OOM"""
    buf = {"q": [], "k": []}
    num_layers = len(model.model.layers)
    layers_to_hook = [num_layers // 4, num_layers // 2, num_layers - 1]
    
    handles = []
    for l_idx in layers_to_hook:
        attn = model.model.layers[l_idx].self_attn
        handles.append(attn.q_proj.register_forward_hook(
            lambda m, inp, out: buf["q"].append(out.detach().cpu().to(torch.float32))))
        handles.append(attn.k_proj.register_forward_hook(
            lambda m, inp, out: buf["k"].append(out.detach().cpu().to(torch.float32))))
    try:
        with torch.no_grad():
            model(ids, use_cache=False)
    finally:
    ...
    L, c = ids.shape[1], model.config
    nh, nk = c.num_attention_heads, getattr(c, "num_key_value_heads", c.num_attention_heads)
    hd = c.hidden_size // nh
    
    total_sal = np.zeros(L, dtype=np.float32)
    for i in range(len(layers_to_hook)):
        q = buf["q"][i].view(1, L, nh, hd).transpose(1, 2)
        k = buf["k"][i].view(1, L, nk, hd).transpose(1, 2)
        if nk != nh: k = k.repeat_interleave(nh // nk, dim=1)
        sc = torch.matmul(q[:, :, -1:, :], k.transpose(-2, -1)) * (hd ** -0.5)
        total_sal += torch.softmax(sc, dim=-1)[0, :, 0, :].mean(0).numpy()
        del q, k, sc
    
    del buf
    return total_sal / len(layers_to_hook)
...
# Policy dispatch — maps name → (signal_key, function)
POLICIES: dict = {
...
try:
    ...
    pass  # fallback LCS already defined above

def compute_metrics(pred: str, ground_truth) -> dict:
    """Multi-answer aware: takes best score over all reference answers."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# COHERENCE LOSS — proxy for eviction language coherence
# ═══════════════════════════════════════════════════════════════════════════════

@torch.no_grad()
def measure_coherence_loss(model, ids: torch.Tensor, keep: set) -> float:
    """
    Cross-entropy loss on evicted token sequence.
    Measures how well the model predicts the kept tokens from left to right —
    lower = better preserved language coherence after eviction.
    Not a true PPL since the spatial adjacency is broken by eviction.
    """
    if not keep:
        return float("inf")
    keep_t    = torch.tensor(sorted(keep), device=ids.device, dtype=torch.long)
...
RULER_BUILDERS: dict = {
    "niah_single":   build_niah_single,   # needs (rng, tok, toks, depth)
    "niah_multikey": build_niah_multikey, # needs (rng, tok, toks)
    "vt":            build_vt,            # needs (rng, tok, toks)
}
...
def _make_results_skeleton(pol_names: list) -> dict:
    """results[source][task][ctx][policy][budget] = {metric: []}"""
    res: dict = {}
...
def run_trial(
    ...
    """
    Runs all 6 policies + FullContext on one (ids, ground_truth) input.
    Returns (trial_data, pred_rows, speed_rows, coherence_rows).
    """
...
def main() -> None:
        ...
        for ctx_len in CTX_LENS:
            ...
            for trial in range(N_TRIALS):
                ...
                if done % 5 == 0:
                    ...
                    print(f"  [{done}/{total}] {elapsed:.1f}min | KiaOmni={ki:.3f} SnapKV_Modified={sk:.3f}", flush=True)
    ...
    # Macro avg F1 across all tasks + contexts
